...
export default function CustomCandleBox() { // Updated function name to reflect Candle Boxes
  const [quantity, setQuantity] = useState('1000 units');
  const [style, setStyle] = useState('Candle Box with Lid'); // Updated default style
  const [size, setSize] = useState('4 x 4 x 6 in'); // Updated default size
  ...
  const calculateCosts = () => {
    ...
    const packagingCostPerUnit = 1.5; // Updated packaging cost per unit
    const shippingCostPerUnit = 0.2; // Updated shipping cost per unit
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-start p-8 bg-gray-50 min-h-screen gap-8 mt-20"> {/* Increased margin-top to prevent being cut by the header */}
      ...
      <div className="w-full lg:w-1/2 flex justify-center items-center">
        <Card className="bg-gray-800 rounded-lg w-4/5 h-4/5 p-6 flex flex-col items-center">
          ...
          <div className="text-center text-white mb-8">
            Custom Candle Packaging Simplified
          </div>
      ...
      <div className="w-full lg:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold text-gray-900">Custom Candle Boxes</h1>

        {/* Style Selection */}
        <SelectOption 
          label="Style" 
          value={style} 
          options={["Candle Box with Lid", "Candle Box without Lid"]} // Updated options
          onChange={setStyle} 
        />
        ...
        <SelectOption 
          label="Size" 
          value={size} 
          options={["4 x 4 x 6 in", "5 x 5 x 7 in", "6 x 6 x 8 in"]} // Updated options
          onChange={setSize} 
        />
