...
export default function CustomPouchBag() {
  const [quantity, setQuantity] = useState('1000 units');
  const [style, setStyle] = useState('Pouch Bag with Zipper');
  ...
  const calculateCosts = () => {
    ...
    const packagingCostPerUnit = 1.2; // Updated cost for pouch bags
    const shippingCostPerUnit = 0.15; // Updated shipping cost
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-start p-8 bg-gray-50 min-h-screen gap-8 mt-20"> {/* Increased margin-top to prevent being cut by the header */}
      ...
      <div className="w-full lg:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold text-gray-900">Custom Pouch Bags</h1>

        {/* Style Selection */}
        <SelectOption 
          label="Style" 
          value={style} 
          options={["Pouch Bag with Zipper", "Pouch Bag with Velcro"]} 
          onChange={setStyle} 
        />
