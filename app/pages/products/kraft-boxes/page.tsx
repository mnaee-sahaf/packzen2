...
export default function CustomKraftBox() {
  const [quantity, setQuantity] = useState('1000 units');
  const [style, setStyle] = useState('Kraft Box with Lid');
  ...
  const calculateCosts = () => {
    ...
    const packagingCostPerUnit = 1.5; // Updated cost for kraft boxes
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-start p-8 bg-gray-50 min-h-screen gap-8 mt-20"> {/* Increased margin-top to prevent being cut by the header */}
      ...
      <div className="w-full lg:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold text-gray-900">Custom Kraft Boxes</h1>

        {/* Style Selection */}
        <SelectOption 
          label="Style" 
          value={style} 
          options={["Kraft Box with Lid", "Kraft Box without Lid"]} 
          onChange={setStyle} 
        />
