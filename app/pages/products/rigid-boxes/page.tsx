...
export default function CustomRigidBox() {
  const [quantity, setQuantity] = useState('1000 units');
  const [style, setStyle] = useState('Rigid Box with Lid');
  ...
  const calculateCosts = () => {
    ...
    const packagingCostPerUnit = 1.5; // Updated cost for rigid boxes
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-start p-8 bg-gray-50 min-h-screen gap-8 mt-20"> {/* Increased margin-top to prevent being cut by the header */}
      ...
      <div className="w-full lg:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold text-gray-900">Custom Rigid Boxes</h1>

        {/* Style Selection */}
        <SelectOption 
          label="Style" 
          value={style} 
          options={["Rigid Box with Lid", "Rigid Box with Magnetic Closure"]} 
          onChange={setStyle} 
        />
        ...
        <SelectOption 
          label="Size" 
          value={size} 
          options={["5 x 5 x 5 in", "7 x 5 x 3 in", "8 x 6 x 4 in", "10 x 8 x 6 in"]} 
          onChange={setSize} 
        />
