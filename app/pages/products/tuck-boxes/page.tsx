...
export default function CustomTuckBox() { // Updated function name to reflect Tuck Boxes
  const [quantity, setQuantity] = useState('1000 units');
  const [style, setStyle] = useState('Tuck Box with Lid'); // Updated default style for Tuck Boxes
  const [size, setSize] = useState('5.5 x 2.5 x 4.5 in'); // Default size remains the same
  ...
  const calculateCosts = () => {
    ...
    const packagingCostPerUnit = 1; // Cost per unit for packaging
    const shippingCostPerUnit = 0.1; // Cost per unit for shipping

    const packaging = quantityValue * packagingCostPerUnit; // Total packaging cost
    const shipping = quantityValue * shippingCostPerUnit; // Total shipping cost
    const total = packaging + shipping; // Total cost
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-start p-8 bg-gray-50 min-h-screen gap-8 mt-20"> {/* Increased margin-top to prevent being cut by the header */}
      ...
      <div className="w-full lg:w-1/2 flex justify-center items-center">
        <Card className="bg-gray-800 rounded-lg w-4/5 h-4/5 p-6 flex flex-col items-center">
          ...
          <div className="text-center text-white mb-8">
            Custom Tuck Boxes Simplified {/* Updated description */}
          </div>
      ...
      <div className="w-full lg:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold text-gray-900">Custom Tuck Boxes</h1> {/* Updated title */}

        {/* Style Selection */}
        <SelectOption 
          label="Style" 
          value={style} 
          options={["Tuck Box with Lid", "Tuck Box with Window", "Tuck Box with Insert"]} // Updated options for Tuck Boxes
          onChange={setStyle} 
        />
