...
export default function CustomGableBox() {
  const [quantity, setQuantity] = useState('1000 units');
  const [style, setStyle] = useState('Gable Box with Handle');
  const [size, setSize] = useState('5 x 5 x 7 in');
  ...
  const calculateCosts = () => {
    ...
    const packagingCostPerUnit = 1.2; // Cost for gable boxes
    const shippingCostPerUnit = 0.15; // Shipping cost for gable boxes
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-center items-start p-8 bg-gray-50 min-h-screen gap-8 mt-20"> {/* Increased margin-top to prevent being cut by the header */}
      ...
      <div className="w-full lg:w-1/2 space-y-6">
        <h1 className="text-3xl font-bold text-gray-900">Custom Gable Boxes</h1>

        {/* Style Selection */}
        <SelectOption 
          label="Style" 
          value={style} 
          options={["Gable Box with Handle", "Gable Box without Handle"]} 
          onChange={setStyle} 
        />
        ...
        <SelectOption 
          label="Size" 
          value={size} 
          options={["5 x 5 x 7 in", "6 x 6 x 8 in", "7 x 7 x 9 in"]} 
          onChange={setSize} 
        />
